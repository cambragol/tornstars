...
#
# $Id: PSO.py,v 1.3 2007/10/04 01:12:49 jasper Exp $
#
...
class PSO(IFF):
	def __init__(self):
		IFF.__init__(self)
		self.types = ["PSO2", "PSO "] # no, i don't know what the difference is either
		self.verbose = 1
		self.callbacks = {  "OHDR": self.ohdr, # object header, one per file
							"SHDR": self.shdr, # surface header, 1+ per file
							"VERT": self.vert, # per surface
							"INDX": self.indx, # per surface
							"DELT": self.dump_chunk, # used for animation, guess: it's a list of changes in the positions of vertices
							"FRAM": self.dump_chunk  # used for animation, guess: it's something to do with frames
						}
...
#		print self.oldverts
	...
	def close(self, dist, v1, v2):
		# are two verticies near to each other?
		res = 0.0
		for v in [0, 1, 2]:
			res += (v1[v] - v2[v]) ** 2
	...
	def shdr(self, type, blob):
		...
		# texture flags:
		#
		# 0x24 is very common 32 & 4
		# 0x21 is rarer		32 & 1 (Truck0_T.PSO: TRK_Pistons)
		#
		...
		if self.verbose:
			print 'nverts:', nverts, 'texcount:', tex_count
			print
	...
	def vert(self, type, blob):
		...
		assert length % vertsz == 0, "Vertsz wrong for %s - this means i don't understand SHDR chunks properly (yet). - %d, %d" % (self.filename, vertsz, length)
...
#	pp = pprint.PrettyPrinter(indent=1, depth=5)
	if (len(sys.argv) > 1):
		...
		print "useage: %s <file.psi> [...]" % (sys.argv[0])
#	pp.pprint(pso)
#	print dir(pso)
