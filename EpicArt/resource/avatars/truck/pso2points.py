...
#
# $Id: pso2points.py,v 1.1 2007/12/05 20:48:15 jasper Exp $
#
# Convert PSO[2 ] -> a points file
#
...
#			(x, y, z, nx, ny, nz) = vert # vert also has normals and texture co-ords.
		...
		for s in surfaces:
			...
			for k in ['red', 'green', 'blue', 'alpha']:
				out[k] = surf[k]
			out = [str(k) + ':' + str(out[k]) for k in out]
			out = ';'.join(out)
			out = 'surf ' + out
			print out
